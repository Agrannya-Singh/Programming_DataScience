...
───────────────
# leaflet()
→ creates an empty HTML widget canvas
# addTiles()
→ applies the default OpenStreetMap base layer
# setView()
→ centres the viewport at Katpadi (VIT); zoom = 10
map1 <- leaflet() %>%
addTiles() %>%
setView(lng = 80.1309,
# longitude: Katpadi, Tamil Nadu
lat = 12.9699,
# latitude: Katpadi, Tamil Nadu
zoom = 10)
# zoom level 10 — city-scale
view
...
───────────────────────
# addCircleMarkers() renders a styled circle instead of a default
pin.
...
# radius → size of the circle in screen pixels (not geographic
units)
# label → persistent text tooltip visible on mouse hover
map3 <- map2 %>%
addCircleMarkers(lng
= 80.2,
# slight eastward offset
from pin
lat
= 12.95,
# slight southward offset
from pin
color = "red",
radius = 8,
